...
def plotar_regiao_viavel(result):
    ...
    # Restrições
    plt.axvline(3, color="blue", label=r"$x_1 \leq 3$")
    plt.axhline(2, color="orange", label=r"$x_2 \leq 2$")
    plt.plot(x_vals, x_vals - 1, "purple", label=r"$x_1 - x_2 \geq 1$")

    # Solução ótima
    if status == "Optimal":
        plt.plot(x1_opt, x2_opt, "ro", markersize=10, label="Ótimo")
        plt.text(x1_opt + 0.2, x2_opt - 0.4, f"({x1_opt:.1f}, {x2_opt:.1f})\nZ={z_opt:.1f}")
